function [ avgSurfIntegral ] = pwbGaussLegendreAverage( kernel , order , weight , isCylindircal , isHemisphere )
%pwbGaussLegendreAverage - Average over the surface of a sphere or hemisphere using
%                          Gauss-Legendre quadrature.
%
% Usage:
%
% [ surfIntegral ] = pwbGaussLegendreAverage( kernel , order , weight , isCylindircal , isHemisphere )
%
% Inputs:
%
...
% Outputs:
%
% avgSurfIntegral - complex/real array, average value(s) of the surface integral [arb].
%
% Notes:
%
...
% Date: 19/01/2017
  ...
  if( ndims( kernel ) == 1 )
    ...
    avgSurfIntegral = pi ./ order .* sum( weight(:) .* kernel(:) ) ./ 4.0 ./ pi ./ size( weight , 3 );   
  elseif( ndims( kernel ) == 2 )
    % Assume first index is flattened PW index.
    if( size( kernel , 2 ) ~= numel( weight ) )
      error( 'two-dimensional kernel must have same number of rows as the number of elements in weight' );
    end % if 
    avgSurfIntegral = pi ./ order .* sum( bsxfun( @times , weight(:)' , kernel ) , 2 ) ./ 4.0 ./ pi ./ size( weight , 3 );
  elseif( ndims( kernel ) == 4 )
    ...
    avgSurfIntegral = pi ./ order .* sum( weight(:)' .* kernel , 2 ) ./ 4.0 ./ pi ./ size( weight , 3 );
  else
    error( 'unsupported kernel shape' );
  end % if
  
  if( isCylindircal )
    avgSurfIntegral = 2.0 * order * avgSurfIntegral;
  end % if

  if( isHemisphere )
    avgSurfIntegral = 2.0 * avgSurfIntegral;
  end % if
