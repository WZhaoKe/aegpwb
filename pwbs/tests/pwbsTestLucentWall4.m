function [ isPass ] = pwbsTestLucentWall4()
% pwbsTestLucentWall4 - 
%
...
% Date: 19/08/2016
  ...
  dlmwrite( 'pwbsTestLucentWall4.asc' , [ f , RCS1_val , RCS2_val , TCS_val ] , ' ' );
  ...
  pwbm = pwbsAddAperture( pwbm , 'LW' , 'C1' , 'C2' , 1 , 'LucentWallFileCCS' , { area , 'pwbsTestLucentWall4.asc' } );
  pwbm = pwbsSolveModel( pwbm );
 
  delete( 'pwbsTestLucentWall4.asc' );
  ...
  isPass = isPass && isValid( data{1} , 0.25 .* (1.0 - RE1_val ) .* area );  
  isPass = isPass && isValid( data{2} , 1.0 - RE1_val ); 
  [ data , units ] = pwbsGetOutput( pwbm , 'Absorber' , 'LW_A2' , { 'ACS' , 'AE' } );
  isPass = isPass && isValid( data{1} , 0.25 .* (1.0 - RE2_val ) .* area );  
  isPass = isPass && isValid( data{2} , 1.0 - RE2_val ); 
  [ data , units ] = pwbsGetOutput( pwbm , 'Cavity' , 'C1' , { 'powerDensity' } );
  [ S1_val , S2_val , ~ , ~ , ~ , ~ ] = pwbCoupledCavities( 0.25 .* (1.0 - RE1_val ) .* area  , 0.25 .* (1.0 - RE2_val ) .* area  , TCS_val , 1.0 , 0.0 );
